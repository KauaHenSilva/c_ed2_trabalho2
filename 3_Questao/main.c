...
void informar_primeiro_nos(Arvore_2_3 **arvore)
{
  ...
  pegar_inteiro_em_intervalo(&inicio, 0, TAMANHO_MEMORIA, "Digite o endereço inicial do nó: ");
  pegar_inteiro_em_intervalo(&final, inicio, TAMANHO_MEMORIA, "Digite o endereço final do nó: ");
  pegar_status_da_memoria(&status);

  set_info(&info_primeiro, inicio, final, status);
...
int main()
{
  ...
  {
    menu();
    pegar_inteiro_em_intervalo(&opcao, 0, 1, "Digite a opção desejada: ");
    if (opcao == 1)
    {
      int tamanho;
