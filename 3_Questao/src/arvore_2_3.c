...
void no_com_tamanho_livre(Arvore_2_3 **no_adequado, int *info_correspodente, Arvore_2_3 *arvore, int tamanho)
{
  ...
  {
    int resultado = 0;
    if ((arvore->info1.status == LIVRE) && ((resultado = arvore->info1.final - arvore->info1.inicio) >= tamanho))
    {
      *no_adequado = arvore;
      *info_correspodente = 1;
    }
    else if ((arvore->qtd_info == 2) && (arvore->info2.status == LIVRE) && (resultado = arvore->info2.final - arvore->info2.inicio) >= tamanho)
    {
      *no_adequado = arvore;
...
void ocupar_no(Arvore_2_3 *arvore, int tamanho)
{
  ...
  int no_valido_info = 0;

  no_com_tamanho_livre(&no_valido, &no_valido_info, arvore, tamanho);
  if (no_valido)
  {
  }
}
  ...
  if ((*raiz)->info1.inicio > info.inicio)
  {
    *info_para_cima = (*raiz)->info2;
    if (*filho)
      ...
      criar_arvore_2_3(novo_no, info, (*raiz)->direita, NULL);
  }
  else if ((*raiz)->info2.inicio < info.inicio)
  {
    *info_para_cima = info;
    if (*filho)
      ...
      criar_arvore_2_3(novo_no, (*raiz)->info2, NULL, (*raiz)->direita);
  }
  else
  {
